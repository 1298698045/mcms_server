		...
		methods: {
			/*changePage: function(page) {
				if (this.action.indexOf('?') == -1) {
					this.action = this.action + '?page=' + page;
				} else {
					this.action = this.action + '&page=' + page;
				}
			},*/
			handleNodeClick: function (data) {
				if (data.categoryType == '1' || data.categoryType == '3') {
					this.action = ms.manager + "/cms/content/main.do?categoryId=" + data.id + "&type=" + data.categoryType;
			...
			treeList: function () {
				...
				ms.http.get(ms.manager + "/cms/category/list.do", {
					...
					if (!res.result || res.data.total <= 0) {
						...
						that.emptyText = '';
						// 过滤掉栏目类型为链接属性
						var children = ms.util.treeData(res.data.rows, 'id', 'categoryId', 'children');
						that.treeData = [{
							id: 0,
							categoryTitle: '全部',
							children: children
